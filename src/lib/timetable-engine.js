...
exports.findFreeTeachers = findFreeTeachers;
exports.getTeacherSlotsForDay = getTeacherSlotsForDay;
exports.getDivisionTimetable = getDivisionTimetable;
function generateTimetable(input, activeConstraints = []) {
    ...
    const divisionDaySubjects = new Map();
    ...
    function resolveTeacherId(ds, div) {
        if (ds.useClassTeacher)
            return div.classTeacherId ?? ds.teacherId;
        return ds.teacherId;
    }
    // Helper: resolve slot number from "FIRST" / "LAST" / numeric string
    function resolveSlot(fixedSlot) {
        if (fixedSlot === 'FIRST')
            return 1;
        if (fixedSlot === 'LAST')
            return slotsPerDay;
    ...
    for (const div of divisions) {
        for (const ds of div.subjects) {
            ...
            if (!teacherSchedule.has(teacherId)) {
                ...
                for (let day = 1; day <= days; day++)
                    teacherSchedule.get(teacherId).set(day, new Set());
                teacherLoad.set(teacherId, 0);
            }
            let periodsPlaced = 0;
            // Determine which days to pin on
            const targetDays = ds.fixedDay
                ? [ds.fixedDay]
                : Array.from({ length: days }, (_, i) => i + 1);
            for (const day of targetDays) {
                if (day > days)
                    continue; // Respect workingDays
                if (periodsPlaced >= ds.periodsPerWeek)
                    break;
                const targetSlot = ds.fixedSlot ? resolveSlot(ds.fixedSlot) : null;
                if (targetSlot !== null) {
                    // Check if slot is free
                    const occupied = timetable.find(t => t.divisionId === div.id && t.dayOfWeek === day && t.slotNumber === targetSlot);
                    if (occupied) {
    ...
    if (enableClassTeacherPeriod1) {
        for (const div of divisions) {
            ...
            const ctSubject = div.subjects.find(s => s.teacherId === div.classTeacherId && !s.fixedDay && !s.fixedSlot);
            if (!ctSubject) {
            ...
            for (let day = 1; day <= days; day++) {
                if (teacherSchedule.get(teacherId)?.get(day)?.has(1))
                    continue;
                const alreadyPlaced = timetable.filter(t => t.divisionId === div.id && t.subjectId === ctSubject.subjectId).length;
                if (alreadyPlaced >= ctSubject.periodsPerWeek)
                    continue;
                if (divisionDaySubjects.get(div.id)?.get(day)?.has(ctSubject.subjectId))
                    continue;
                // Check slot 1 is free in this division
                if (timetable.find(t => t.divisionId === div.id && t.dayOfWeek === day && t.slotNumber === 1))
                    continue;
    ...
    for (const div of divisions) {
        for (const ds of div.subjects) {
            ...
            if (!teacherId)
                continue;
            const alreadyPlaced = timetable.filter(t => t.divisionId === div.id && t.subjectId === ds.subjectId).length;
            const remaining = ds.periodsPerWeek - alreadyPlaced;
            if (remaining <= 0)
                continue;
            let priority = 0;
            if (ds.isCore && !ds.eveningPriority)
                priority = 1;
            else if (ds.isCore)
                priority = 2;
            else if (!ds.eveningPriority)
                priority = 3;
            else
                priority = 4;
            if (ds.periodsPerWeek >= 5)
                priority -= 0.5;
            if (ds.consecutiveSlots > 1)
                priority -= 0.3;
            if (ds.fixedDay || ds.fixedSlot)
                continue; // already handled in Step 0
            assignments.push({
                divisionId: div.id,
    ...
    for (const assignment of assignments) {
        ...
        if (assignment.eveningPriority && enableEveningPriority) {
            for (let s = morningPeriods + 1; s <= slotsPerDay; s++)
                preferredSlots.push(s);
            for (let s = 2; s <= morningPeriods; s++)
                fallbackSlots.push(s);
        }
        ...
        while (periodsAssigned < assignment.periodsNeeded) {
            ...
            for (const day of dayOrder) {
                ...
                if (enableMaxOnceDay && divisionDaySubjects.get(assignment.divisionId)?.get(day)?.has(assignment.subjectId))
                    continue;
            ...
            if (!placed) {
                ...
                if (!forcePlaced) {
                    errors.push(`Could not assign all periods for subject in ${assignment.divisionName}. ` +
    ...
    const totalExpected = divisions.reduce((sum, div) => sum + div.subjects.reduce((s, ds) => s + ds.periodsPerWeek, 0), 0);
    return {
...
// ── Helpers ────────────────────────────────────────────────────────────────
function canPlaceSlot(timetable, teacherSchedule, assignment, day, slot, enableNoSameTeacherConsecutive = false) {
    if (timetable.find(t => t.divisionId === assignment.divisionId && t.dayOfWeek === day && t.slotNumber === slot))
        return false;
...
function canPlaceConsecutiveSlots(timetable, teacherSchedule, assignment, day, startSlot, count, maxSlots, morningPeriods, enableNoSpanLunch, enableNoSameTeacherConsecutive = false) {
    // Don't span across lunch break
    if (enableNoSpanLunch && startSlot <= morningPeriods && startSlot + count - 1 > morningPeriods)
        return false;
    if (startSlot + count - 1 > maxSlots)
        return false;
    for (let i = 0; i < count; i++) {
        if (!canPlaceSlot(timetable, teacherSchedule, assignment, day, startSlot + i, enableNoSameTeacherConsecutive))
            return false;
    }
    return true;
}
...
// ── Public utilities ───────────────────────────────────────────────────────
function findFreeTeachers(timetable, allTeacherIds, day, slot) {
    const busyTeachers = new Set(timetable.filter(t => t.dayOfWeek === day && t.slotNumber === slot).map(t => t.teacherId));
    return allTeacherIds.filter(id => !busyTeachers.has(id));
}
function getTeacherSlotsForDay(timetable, teacherId, day) {
    return timetable.filter(t => t.teacherId === teacherId && t.dayOfWeek === day);
}
function getDivisionTimetable(timetable, divisionId) {
